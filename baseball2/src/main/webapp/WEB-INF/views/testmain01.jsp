...
div.modalContent th.text-tg-join {
	text-align: right;
}
...
<figure class="highcharts-figure">
  ...
  <table id="datatable">
	...
	<div class="side-bar-hover">
		<div class="hover-box">
			<em> <span>O</span> <span>H</span> <span>!</span> <span>-</span>
				<span>Y</span> <span>E</span> <span>S</span><br>
			</em>
		...
		<div class="phone-number">
			...
			<div class="panel-heading">
				<c:if test="${id=='' || id==null}">
					...
					<form class="form-inline" action="<c:url value='/memberLogin.do'/>"
						method="post">
						...
						<div class="email">
							<button style="position: relative; left: -20px;" type="submit"
								class="boardselect boardselect_member" onclick="return check()">로그인</button>
							<button style="position: relative; left: -20px;" type="button"
								class="boardselect boardselect_member" onclick="signUp()">회원가입</button>
						</div>
					</form>
				</c:if>
				<!-- 로그인 성공 시 활성화 -->
				<c:if test="${id!='' && id!=null}">
					<div class="form-group">
						...
						<br>
						<button style="position: absolute; bottom: 70px; right: 5px;"
							type="button" class="boardselect boardselect_member"
							onclick="out()">로그아웃</button>
						<br>
					</div>
					<div class="email">
						<span style="position: absolute; right: 15px;">개인정보 관리</span> <br>
						<button style="position: absolute; top: 15px; right: 5px;"
							type="button" class="boardselect boardselect_member"
							onclick="contentpg()">회원정보 수정</button>
						<br>
						<button style="position: absolute; top: 45px; right: 5px;"
							type="button" class="boardselect boardselect_member"
							onclick="memderdel()">회원 탈퇴</button>
						<br>
					</div>
				</c:if>
			</div>
		...
		<div class="box-wrap">
			...
			<div class="menu">
				<div class="o_container">
					<div class="menu-left">
						<a href="" target="_blank"><span> </span></a> <a
							href="https://sports.news.naver.com/kbaseball/schedule/index.nhn"
							target="_blank"><span>경기 일정</span><i class="fa fa-share"
							aria-hidden="true"></i></a><br>
					</div>
					<!-- 클릭시 이동하는 부분 -->
					<div class="menu-right">
						<a href="#section1"><span>메인페이지</span></a><br> <a
							href="#section2"><span>게시판</span></a><br> <a
							href="#section3"><span>경기예측</span></a><br> <a
							href="#section4"><span>연봉예측</span></a><br> <a
							href="#section5"><span>관리정보</span></a><br>
					</div>
				</div>
			</div>
		</div>
					...
					<div class="board_list" style="padding-top: 10%;">
						...
						<table class="table table-hover table-bordered">
							...
							<c:forEach var="vo" items="${list}">
								<tr>
									...
									<td>${vo.send_date}</td>
									<td><a
										href="<c:url value='/boardContent.do'/>?num=${vo.num}">내용
											보기 </a> <c:if test="${id eq vo.writer}">
											<a href="<c:url value='/boardDelete.do'/>?num=${vo.num}">/
												삭제</a>
										</c:if></td>
								</tr>
							</c:forEach>
						...
						<c:if test="${id!='' && id!=null}">
							<input type="button" value="글쓰기" class="btn btn-primary"
								onclick="goForm()" style="position: absolute; margin-left: 45%;" />
						</c:if>
					</div>
					...
					<c:if test="${id!='' && id!=null}">
						<div class="about" style="width: 245px;">
							<form action="<c:url value='/boardList.do'/>">
								<button type="submit" class="boardselect boardselect_list">전체
									글</button>
							</form>
							<button type="button" class="boardselect boardselect_list"
								onclick="memListView()">작성 글</button>
						</div>
					</c:if>
					...
					<div class="s3-cont1 cont-wrap" id="cont1">
						<div class="side-text-pl side-left">
							...
							<div class="h3_tit_pl cont1">
								<span>STEP 1. 날짜 선택하기 </span><br> <input type="text"
									id="datepicker" class="select-data">
							</div>
							<div class="h3_tit_pl cont1">
								<span>STEP 2. 지역 선택하기 </span><br> <select
									class="select-data location">
							...
							<div class="h3_tit_pl cont1">
								<span>STEP 3. 팀 선택하기 </span><br> <span>HOME . </span> <select
									onchange="homeselectteam(this)" class="select-data home_team">
								...
								</select> <a style="display: none;" id="selecthome"></a> <br> <span>AWAY
									. </span> <select onchange="awayselectteam(this)"
									class="select-data away_team">
									<option value="">팀 선택</option>
									<option value="KIA">기아 타이거즈</option>
									<option value="DU">두산 베어스</option>
									<option value="LG">LG 트윈스</option>
									...
									<option value="KH">키움 히어로즈</option>
									<option value="KT">KT 위즈</option>
								</select> <a style="display: none;" id="selectaway"></a>
							</div>
							<c:choose>
								<c:when test="${id!='' && id!=null}">
									<button type="button" id="submit-python"
										class="boardselect baseballdeep">결과 보기</button>
								</c:when>
								<c:when test="${id=='' || id==null}">
									<button type="button" class="boardselect baseballdeep"
										onclick="alert('로그인 시 이용 가능합니다.');">결과 보기</button>
								</c:when>
							</c:choose>
					...
					<div class="s3-cont2 cont-wrap" id="cont2">
						...
						<div class="side-text side-left">
							...
							<h3 class="h3_tit cont2">
								<img id="home-emblem-img" class="emblem-st-img"
									src="assets/baseball/kbo_emblem.jpg"> <span
									style="font-size: 5rem;" id="result_text"></span>
							</h3>
						...
						<div class="h3_tit" style="height: 500px; text-align: center;">
							<br> <br> <br> <span>VS</span>
						</div>

						<div class="side-text-pl side-right">
							<p class="s1_tit cont_pl">AWAY</p>
							<h3 class="h3_tit cont1_pl">
								<img id="away-emblem-img" class="emblem-st-img"
									src="assets/baseball/kbo_emblem.jpg"> <span
									style="font-size: 5rem;" id="result"></span>
							</h3>
						...
						<div class="ani-wrap clearfix">
							<div>
								<select id="address2" class="select-data away_team"
									...
									<option value="KH">키움 히어로즈</option>
									<option value="KT">KT 위즈</option>
								</select> <select id="address3"
									class="baseball-member-salary select-data"
									onchange="playerPredictData();">
							...
							<div class="desc desc-left">
								<h5>선수 정보</h5>
								<p>
									<strong>이름</strong> <span id="plaryName"></span><br> <strong>출생</strong>
									<span id="birth"></span><br> <strong>신체</strong> <span
										id="bady-profile"></span><br> <strong>소속팀</strong> <span
										id="affiliation-team"></span><br>
								</p>
							</div>
							<div class="iframe iframe-left">
								<img id="plarypicture" src="assets/baseball/kbo_emblem.jpg"
			...
			<div class="o_container">
				<div class="sec5">
					...
					<div class="sec5-title show">
						<span>SMHRD BigData</span> <span>3차 프로젝트</span>
					</div>
					<div class="contact-img show"></div>
					<div class="contactMe-left show">
						<span>60, Songam-ro, Nam-gu, Gwangju, Republic of Korea</span>
					</div>
					<div class="contactMe-right show">
						<div class="home">
							<strong>HOME</strong><br> <strong>062-1234-5678</strong>
						</div>
						<div class="tell">
						...
						<div class="email">
							<strong>E-MAIL US</strong><br> <strong>shmrd@gmail.com</strong>
						</div>
					</div>
				</div>
			</div>
				...
				<form action="<c:url value='/memberInsert.do'/>" method="post">
					<table class="table table-bordered">
						<tr>
							...
							<th class="tg-0pky" colspan="2"><input
								style="width: 70%; float: left;" type="text" minlength="3"
								maxlength="20" name="id" id="userid" />
								<button style="float: right;" type="button" value="중복확인"
									class="boardselect idchecksubmit" id="idck">중복 확인</button></th>
						</tr>
						<tr>
							<th class="text-tg-join">비밀번호 :</th>
							<th class="tg-0pky"><input type="password" name="pw"
								id="userpw" /></th>
						</tr>
						<tr>
						...
						<tr>
							...
							<th class="tg-0lax"><select name="ft">
						...
						<tr>
							<th class="tg-0lax" colspan="2" align="center">
								<button type="submit" value="가입"
									class="boardselect boardselect_list" onclick="return idchtr()">가입</button>
								<button type="reset" value="취소"
									class="boardselect boardselect_list" onclick="pageclose()">취소</button>
							</th>
						</tr>
					</table>
				</form>
			...
			<div class="modalContent modeldel">
				<form action="<c:url value='/memberDelete.do'/>" method="post">
					...
					<table class="table table-bordered del">
						<tr>
							<th colspan="2" align="center">${name}님탈퇴하시겠습니까</th>
						</tr>
						<tr>
							<th colspan="2" align="center">
								<button type="submit" value="탈퇴"
									class="boardselect boardselect_list">탈퇴</button>
								<button type="reset" value="취소"
									class="boardselect boardselect_list" onclick="pageclose()">취소</button>
							</th>
						</tr>
	...
	<!-- 팀 select문 동작 -->
	<script src="assets/js/pythondeeplearning.js"></script>
	<!-- 플라스크 -->
	<script src="assets/js/player-predict.js"></script>
	<!-- 선수연봉 -->
